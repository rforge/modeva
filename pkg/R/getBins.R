getBins <- function (obs = NULL, pred = NULL, model = NULL, id = NULL, 
...
min.prob.interval = 0.1, simplif = FALSE) 
  {
    ...
    if (bin.method == "round.prob") {
      ...
      prob.bin <- round(pred, digits = nchar(min.prob.interval) - 
                          2)
    }
    if (bin.method == "prob.bins") {
      message("Arguments n.bins and min.bin.size are ignored by this bin.method")
    ...
    if (bin.method == "n.bins") {
      ...
      if (fixed.bin.size) {
        prob.bin <- findInterval(pred, quantile(pred, 
                                                probs = seq(from = 0, 
                                                to = 1, by = 1/n.bins)))
      }
      else {
    ...
    bins.table <- data.frame(rowSums(bins.table), bins.table[, 
                                                             c(2, 1)])
    ...
    bins.table$adj.diff <- with(bins.table, abs(difference - 
                                                  max.poss.diff))
    bins.table$overpred <- apply(bins.table[, c("prevalence", 
                                                "mean.prob")], 1, max)
    bins.table$underpred <- apply(bins.table[, c("prevalence", 
                                                 "mean.prob")], 1, min)
    ...
    return(list(prob.bin = prob.bin, bins.table = bins.table, 
                N = N, n.bins = n.bins))
  }